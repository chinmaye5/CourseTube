        ...
        <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500">Manage CourseTube ecosystem</p>
                        </div>
                    </div>
                    <div className="flex items-center space-x-6">
                        <div className="hidden md:flex bg-card-theme p-1 rounded-xl border border-border-theme">
                            <button
                                onClick={() => setActiveTab('analytics')}
                                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'analytics' ? 'bg-indigo-500 text-white shadow-lg' : 'text-slate-500 hover:text-foreground'}`}
                            >
                                Analytics
                            </button>
                            <button
                                onClick={() => setActiveTab('messages')}
                                className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'messages' ? 'bg-indigo-500 text-white shadow-lg' : 'text-slate-500 hover:text-foreground'}`}
                            >
                                Messages
                                {messages.length > 0 && <span className="w-2 h-2 bg-rose-500 rounded-full" />}
                            </button>
                        </div>
                        <ThemeToggle />
            ...
            <main className="max-w-7xl mx-auto px-6 py-8">
                {activeTab === 'analytics' ? (
                    ...
                    <div className="bg-surface-theme border border-border-theme rounded-3xl overflow-hidden shadow-xl">
                        <div className="p-8 border-b border-border-theme">
                            <h3 className="text-2xl font-bold flex items-center gap-3">
                                <Mail className="text-indigo-500" />
                                User Inquiries
                            </h3>
                            <p className="text-slate-500 mt-1">Direct messages from the contact form</p>
                        </div>
                        <div className="p-8">
                            {fetchingMessages ? (
                                ...
                                <div className="space-y-6">
                                    {messages.map((msg) => (
                                        <div key={msg._id} className="bg-card-theme/30 border border-border-theme p-6 rounded-2xl hover:border-indigo-500/30 transition-all group">
                                            <div className="flex justify-between items-start mb-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center font-bold text-indigo-500 uppercase">
                                                        {msg.name[0]}
                                                    </div>
                                                    <div>
                                            ...
                                            <div className="bg-background/50 p-4 rounded-xl border border-border-theme/50">
                                ...
                                <div className="py-20 text-center border-2 border-dashed border-border-theme rounded-3xl">
                                    ...
                                    <h4 className="text-xl font-bold text-slate-600">No messages yet</h4>
                                    <p className="text-slate-500 max-w-xs mx-auto mt-2">
                                        When users reach out through the contact form, their messages will appear here.
                                    </p>
                                </div>
                            )}
                        </div>
